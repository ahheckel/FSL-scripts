...
#export POSIXLY_CORRECT # HKL
unset POSIXLY_CORRECT # added by HKL to please recon-all
...
###########################################################################
	...
	if [ "x$tasks" = "x" ] ; then
		...
		sge_command="qsub -V -cwd -shell n -b y -r y $queueCmd $pe_options -M $mailto -N $JobName -m $MailOpts $LogOpts $sge_arch $sge_hold $sge_rsc" # sge_rsc added by HKL
	    else
		sge_command="qsub $LogOpts $sge_arch $sge_hold $sge_rsc" # sge_rsc added by HKL
	    fi
	    ...
	    sge_command="qsub -V -cwd $queueCmd $pe_options -M $mailto -N $JobName -m $MailOpts $LogOpts $sge_arch $sge_hold $sge_tasks $sge_rsc" # sge_rsc added by HKL
	    if [ $verbose -eq 1 ] ; then 
		echo sge_command: $sge_command >&2
