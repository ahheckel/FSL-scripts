...
for i=1:ncon
  ...
  disp(sprintf('\ncontrast %d, best values: uncorrected_p=%f (at %i) FWE_corrected_p=%f (at %i). \nFDR-correction-threshold=%f (to be applied to uncorrected p-values)\n',i,1-max(p_uncorrected(i,:)),idx_u(1),1-max(p_corrected(i,:)),idx_c(1),FDRthresh)) % added (HKL)
