...
#POSIXLY_CORRECT=1 # rem by HKL
#export POSIXLY_CORRECT # rem by HKL
command=`basename $0`
...
###########################################################################
	...
	if [ "x$tasks" = "x" ] ; then
		...
		sge_command="qsub -V -cwd -shell n -b y -r y -q $queue $pe_options -M $mailto -N $JobName -m $MailOpts $LogOpts $sge_arch $sge_hold $sge_rsc" # sge_rsc added by HKL
	    else
	    ...
	    sge_command="qsub -V -cwd -q $queue $pe_options -M $mailto -N $JobName -m $MailOpts $LogOpts $sge_arch $sge_hold $sge_tasks $sge_rsc" # sge_rscadded by HKL
	    if [ $verbose -eq 1 ] ; then 
		echo sge_command: $sge_command >&2
