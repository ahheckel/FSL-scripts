...
Usage() {
    echo ""
    echo "Usage: `basename $0` <input4D> <output4D> <magn img> <dphase img> <dphaseTE(s)> <TE(ms)> <ESP(ms)> <unwarp direction: x/y/z/x-/y-/z-> <interp (default:trilinear)> <subj_idx> <sess_idx>"
...
echo "`basename $0` :  motion-correcting and unwarping input 4D '$(basename $input)'..."
