...
do_reg(){
	...
	if [ ! -f ${o}_warp.msf ] ; then
	    echo $o
	    touch ${o}_warp.msf
      if [ -f $(remove_ext $g).mat ] ; then
        echo "'$(remove_ext $g).mat' detected." # added by HKL
        echo "$FSLDIR/bin/fsl_reg $g $f ${g}_to_$f -e -FA -flirt \"-init $(remove_ext $g).mat -nosearch -dof 6\"" >> .commands # added by HKL
      else
