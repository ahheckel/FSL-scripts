...
Usage() {
    echo ""
    echo "Usage: `basename $0` <out-dir> <dwi-plus> <dwi-minus> <TotalReadoutTime(s)> <use noec> <use ec> [<dof> <costfunction>]"
    echo ""
    exit 1
}
  ...
  for subj in `cat .subjects` ; do
    for sess in `cat ${subj}/.sessions_struc` ; do
      ...
      if [ "x$pttrn_diffsplus" = "x" -o "x$pttrn_diffsminus" = "x" -o "x$pttrn_bvalsplus" = "x" -o "x$pttrn_bvalsminus" = "x" -o "x$pttrn_bvecsplus" = "x" -o "x$pttrn_bvecsminus" = "x" ] ; then
        echo "TOPUP : out-dir '$outdir' : ERROR : file search pattern for blipUp/blipDown DWIs not set..."
        continue
      fi
      ...
      # display info
      echo "TOPUP : out-dir '$outdir' : preparing TOPUP... "
      ...
      if [ ! $n_plus -eq $n_minus ] ; then 
        echo "TOPUP : out-dir '$outdir' : ERROR : number of +blips diff. files ($n_plus) != number of -blips diff. files ($n_minus) - continuing loop..."
        continue
      elif [ $n_plus -eq 0 -a $n_minus -eq 0 ] ; then
        echo "TOPUP : out-dir '$outdir' : ERROR : no blip-up/down diffusion files found for TOPUP (+/- must be part of the filename) - continuing loop..."
        continue
      fi
      ...
      if [ ! $n_vec_plus -eq $n_vec_minus ] ; then 
        echo "TOPUP : out-dir '$outdir' : ERROR : number of +blips bvec-files ($n_vec_plus) != number of -blips bvec-files ($n_vec_minus) - continuing loop..."
        continue
      elif [ $n_vec_plus -eq 0 -a $n_vec_minus -eq 0 ] ; then
        echo "TOPUP : out-dir '$outdir' : ERROR : no blip-up/down bvec-files found for TOPUP (+/- must be part of the filename) - continuing loop..."
        continue
      fi
      
      #  are the +/- bval-files equal in number ?
      if [ ! $n_val_plus -eq $n_val_minus ] ; then 
        echo "TOPUP : out-dir '$outdir' : ERROR : number of +blips bval-files ($n_val_plus) != number of -blips bval-files ($n_val_minus) - continuing loop..."
        continue
      elif [ $n_val_plus -eq 0 -a $n_val_minus -eq 0 ] ; then
        echo "TOPUP : out-dir '$outdir' : ERROR : no blip-up/down bval-files found for TOPUP (+/- must be part of the filename) - continuing loop..."
        continue
      fi
      ...
      if [ $nbvalsplus = $nbvalsminus -a $nbvalsplus = $n_entries -a $nbvecsplus = `echo "3*$n_entries" | bc` -a $nbvecsplus = $nbvecsminus ] ; then
        echo "TOPUP : out-dir '$outdir' : number of entries in bvals- and bvecs files consistent ($n_entries entries)."
      else
        echo "TOPUP : out-dir '$outdir' : ERROR : number of entries in bvals- and bvecs files NOT consistent - continuing loop..."
      ...
      for bval in `cat $fldr/bvalsplus_concat.txt` ; do
        if [ $bval != $(cat $fldr/bvalsminus_concat.txt | cut -d " " -f $i)  ] ; then 
          echo "TOPUP : out-dir '$outdir' : ERROR : +bval entries do not match -bval entries (they should have the same values !) - exiting..."
          exit
        fi        
        i=$[$i+1]
      done
      ...
      echo "TOPUP : out-dir '$outdir' : creating index file for TOPUP..."      
      ...
      echo "TOPUP : out-dir '$outdir' : merging diffs... "
    ...
    for subj in `cat .subjects` ; do
      for sess in `cat ${subj}/.sessions_struc` ; do
        ...
        if [ ! -z "$(ls $fldr/ec_diffs_merged_???.ecclog 2>/dev/null)" ] ; then    
          echo "TOPUP : out-dir '$outdir' : WARNING : eddy_correct logfile(s) from a previous run detected - deleting..."
        ...
        for i in `seq -f %03g 001 $(cat $fldr/diff.files | wc -l)` ; do # note: don't use seq -w (bash compatibility issues!) (!)
          ...
          # eddy-correct
          echo "TOPUP : out-dir '$outdir' : eddy_correction of '$dwifile' (ec_diffs_merged_${i}) is using volume no. $b0img as B0 (val:${min})..."
    ...
    for subj in `cat .subjects` ; do
      for sess in `cat ${subj}/.sessions_struc` ; do
        ...
        for i in `seq -f %03g 001 $(cat diff.files | wc -l)` ; do # note: don't use seq -w (bash compatibility issues!) (!)
          echo "TOPUP : out-dir '$outdir' : plotting ec_diffs_merged_${i}.ecclog..."
...
if [ $TOPUP_STG2 -eq 1 ] ; then
  ...
  for subj in `cat .subjects` ; do
    for sess in `cat ${subj}/.sessions_struc` ; do
      ...
      if [ ! -f $fldr/$(subjsess)_acqparam.txt ] ; then echo "TOPUP : out-dir '$outdir' : ERROR : parameter file $fldr/$(subjsess)_acqparam.txt not found - continuing loop..." ; continue ; fi
      
      # display info
      echo "TOPUP : out-dir '$outdir' : concatenate bvals... "
      ...
      echo "TOPUP : out-dir '$outdir' : minimum b-value in merged diff. is $min"
      ...
      echo "TOPUP : out-dir '$outdir' : creating index file for TOPUP (only low-B images)..."      
      ...
      echo "TOPUP : out-dir '$outdir' : creating index file for TOPUP (only the first low-B image in each dwi-file)..." 
      c=0 ; _nvol=0 ; nvol=0
      ...
      lowbs=""
      for b0idx in $b0idces ; do    
        echo "TOPUP : out-dir '$outdir' : found B0 images in merged diff. at pos. $b0idx (val:${min}) - extracting..."
...
if [ $TOPUP_STG3 -eq 1 ] ; then
  ...
  for subj in `cat .subjects` ; do
    for sess in `cat ${subj}/.sessions_struc` ; do
      ...
      # merge B0 images
      echo "TOPUP : out-dir '$outdir' : merging low-B images..."
...
if [ $TOPUP_STG4 -eq 1 ] ; then
  ...
  for subj in `cat .subjects` ; do
    for sess in `cat ${subj}/.sessions_struc` ; do
      ...
      # execute TOPUP
      echo "TOPUP : out-dir '$outdir' : executing TOPUP on merged low-b volumes..."
...
if [ $TOPUP_STG5 -eq 1 ] ; then
  ...
  for subj in `cat .subjects` ; do
    for sess in `cat ${subj}/.sessions_struc` ; do
      ...
      if [ $TOPUP_USE_NATIVE -eq 1 ] ; then
        echo "TOPUP : out-dir '$outdir' : applying warps to native DWIs..."
        fsl_sub -l $logdir -N topup_applytopup_$(subjsess) -t $fldr/applytopup.cmd
      fi
      if [ $TOPUP_USE_EC -eq 1 ] ; then
        echo "TOPUP : out-dir '$outdir' : applying warps to eddy-corrected DWIs..."
  ...
  for subj in `cat .subjects` ; do
    for sess in `cat ${subj}/.sessions_struc` ; do
      ...
      if [ $TOPUP_USE_NATIVE -eq 1 ] ; then
        echo "TOPUP : out-dir '$outdir' : merging topup-corrected DWIs..."
        fslmerge -t $fldr/$(subjsess)_topup_corr_merged $(imglob $fldr/*_topup_corr)
      fi
      if [ $TOPUP_USE_EC -eq 1 ] ; then
        echo "TOPUP : out-dir '$outdir' : merging topup-corrected & eddy-corrected DWIs..."
        fslmerge -t $fldr/$(subjsess)_topup_corr_ec_merged $(imglob $fldr/*_topup_corr_ec)
      fi
      
      # remove negative values
      echo "TOPUP : out-dir '$outdir' : zeroing negative values in topup-corrected DWIs..."
  ...
  for subj in `cat .subjects` ; do
    for sess in `cat ${subj}/.sessions_struc` ; do
      ...
      if [ $f = "mod" ] ; then
        ...
        echo "TOPUP : out-dir '$outdir' : betting B0 image with fi=${f} - extracting B0..."
        if [ ! -f $fldr/lowb.idx ] ; then echo "TOPUP : out-dir '$outdir' : ERROR : low-b index file '$fldr/lowb.idx' not found - continuing loop..." ; continue ; fi
        fslroi $fldr/diffs_merged $fldr/nodif $(sed -n 1p $fldr/lowb.idx) 1
        echo "TOPUP : out-dir '$outdir' : ...and betting B0..."
      ...
      if [ $TOPUP_USE_EC -eq 1 ] ; then
        if [ -z "$(ls $fldr/ec_diffs_merged_???.ecclog 2>/dev/null)" ] ; then 
          echo "TOPUP : out-dir '$outdir' : ERROR : *.ecclog file(s) not found, but needed to rotate b-vectors -> skipping this part..." 
        
        else 
          for i in `seq -f %03g 001 $(cat $fldr/diff.files | wc -l)` ; do
            bvecfile=`sed -n ${i}p $fldr/bvec.files`
            echo "TOPUP : out-dir '$outdir' : rotating '$bvecfile' according to 'ec_diffs_merged_${i}.ecclog'"
      ...
      for i in `seq -f %03g 001 $(cat $fldr/diff.files | wc -l)` ; do # for each run do...        
        ...
        cd $fldr
          ...
          echo "TOPUP : out-dir '$outdir' : create rotation matrices '$(basename $fname_mat)' ($nscans entries) for 6-parameter TOPUP motion correction (angles: $rots)..."
        ...
        echo "TOPUP : out-dir '$outdir' : CHECK rotation angles - topup input: $(printf ' %0.6f' $rots)"
        echo "TOPUP : out-dir '$outdir' : CHECK rotation angles - avscale out: $(avscale --allparams $fldr/check.mat | grep "Rotation Angles" | cut -d '=' -f2)"
        rm $fldr/check.mat
        
        # apply the rotation matrix to b-vector file
        if [ $TOPUP_USE_NATIVE -eq 1 ] ; then 
          bvecfile=`sed -n ${i}p $fldr/bvec.files`
          echo "TOPUP : out-dir '$outdir' : apply rotation matrices '$(basename $fname_mat)' to '`basename $bvecfile`' -> 'bvecs_topup_${i}.rot'"
          xfmrot $fldr/$fname_mat $bvecfile $fldr/bvecs_topup_${i}.rot
        fi        
        if [ $TOPUP_USE_EC -eq 1 ] ; then
          bvecfile=$fldr/bvecs_ec_${i}.rot
          echo "TOPUP : out-dir '$outdir' : apply rotation matrices '$(basename $fname_mat)' to '`basename $bvecfile`' -> 'bvecs_topup_ec_${i}.rot'"
      ...
      for i in `seq -f %03g 001 $nplus` ; do
        ...
        if [ $TOPUP_USE_NATIVE -eq 1 ] ; then 
          echo "TOPUP : out-dir '$outdir' : averaging rotated blip+/blip- b-vectors (no eddy-correction)..."
          average $fldr/bvecs_topup_${i}.rot $fldr/bvecs_topup_${j}.rot > $fldr/avg_bvecs_topup_${i}.rot
        fi
        if [ $TOPUP_USE_EC -eq 1 ] ; then
          echo "TOPUP : out-dir '$outdir' : averaging rotated blip+/blip- b-vectors (incl. eddy-correction)..."
      ...
      if [ $TOPUP_USE_NATIVE -eq 1 ] ; then      
       echo "TOPUP : out-dir '$outdir' : concatenate averaged and rotated b-vectors (no eddy-correction)..."
       concat_bvecs "$fldr/avg_bvecs_topup_???.rot" $fldr/avg_bvecs_topup.rot
      fi      
      if [ $TOPUP_USE_EC -eq 1 ] ; then
        echo "TOPUP : out-dir '$outdir' : concatenate averaged and rotated b-vectors (incl. eddy-correction)..."
        concat_bvecs "$fldr/avg_bvecs_topup_ec_???.rot" $fldr/avg_bvecs_topup_ec.rot
      fi
      
      # display info
      echo "TOPUP : out-dir '$outdir' : dtifit is estimating tensor model using nodif_brain_${f}_mask..."
      
      # estimate tensor model (rotated bvecs)
      if [ $TOPUP_USE_NATIVE -eq 1 ] ; then           
        echo "TOPUP : out-dir '$outdir' : dtifit is estimating tensor model with rotated b-vectors (no eddy-correction)..."
      ...
      if [ $TOPUP_USE_EC -eq 1 ] ; then
        echo "TOPUP : out-dir '$outdir' : dtifit is estimating tensor model with rotated b-vectors (incl. eddy-correction)..."
